...
var _blog = await jitiImport("./data/blog.js");
var _topics = await jitiImport("./data/knowledge/topics.js");
var _fs = await jitiImport("fs");function _interopRequireDefault(e) {return e && e.__esModule ? e : { default: e };}

const blogSlugs = _blog.articles.map((article) => article.slug).filter((slug) => Boolean(slug));

const blogRoutes = blogSlugs.map((slug) => `/blog/${slug}`);

const blogClusters = Array.from(
  new Set(_blog.articles.map((article) => article.cluster).filter((cluster) => Boolean(cluster)))
).map((cluster) => `/blog/cluster/${cluster}`);

// Маршруты для базы знаний
const knowledgeRoutes = _topics.topics.map((topic) => `/knowledge/${topic.slug}`);

// Маршруты для постов базы знаний — читаем JSON динамически
const postsDataPath = new URL('./data/knowledge/posts-data.json', "file:///Users/artemselifanov/qwen/%D0%BB%D0%B5%D0%BD%D0%B4%D0%B8%D0%BD%D0%B3%20ai/nuxt.config.ts");
const postsData = JSON.parse((0, _fs.readFileSync)(postsDataPath, 'utf-8'));
